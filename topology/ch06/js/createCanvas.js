...
function drawHost($c,_x,_y,_w,_h,hostOffset,hosts,event){
    ...
    hosts.forEach(function(v){
        ...
        var net_arr = v.net_id;
        net_arr.forEach(function(w){
            var x = hostOffset[w].x+config.host.l+config.host.w/2;

            var y = _y+config.hostTop*2+config.router.t+config.router.h+config.router.lineWidth+hostOffset[w].host*(config.host.h+config.host.t);
            ...
            $c.addLayer({
                ...
                y1: (y-hostOffset[w].host*(config.host.h+config.host.t))*config.multiple,
